...
| ------------------------------------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| PDF size, exact 25 MB boundary and one extra byte | `src/domain/ingestion.test.ts`; `features/ingestion.feature`                     | Domain tests passed; full browser boundary checked by local BDD                         |
| Invalid file type and renamed non-PDF bytes       | `src/server/ingestion.test.ts`; `src/server/upload-routes.test.ts`               | Validation tests passed; real parser rejects renamed input                              |
...
| Invalid URL / protocol / video identifier         | `src/domain/ingestion.test.ts`                                                   | HTTP/S and exact identifier validation tests passed                                     |
| Unavailable captions                              | `src/server/providers.test.ts`; `services/transcript/test_app.py`                | Explicit error mapping and deterministic no-captions case tested                        |
...
| Cancellation, source replacement, late response   | `tests/e2e/session-races.spec.ts`; `src/lib/page-lifecycle.test.tsx`             | Fault-injection regression and real React unmount tests passed                          |
| Security / server credentials                     | `src/server/response.test.ts`; `infrastructure/scripts/check-client-secrets.mjs` | Ephemeral contract tests and final canary build/scan passed                             |
...
| Native iOS and Android states                     | `apps/mobile/VERIFICATION.md`                                                    | Bundle/unit checks and installed iOS mock UI passed; Android/audio gates remain pending |
