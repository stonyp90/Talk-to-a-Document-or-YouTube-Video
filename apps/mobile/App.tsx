import { DEFAULT_LANGUAGE, Language, TranslationKey, translate, samples } from './src/i18n';
...
function messageOf(error: string, t: (key: TranslationKey) => string) {
  const message = error;
  if (/valid YouTube URL/i.test(message)) return t("This doesn’t look like a YouTube link. Check it and try again.");
  if (/No captions/i.test(message)) return t("This video has no captions. Try another video or a PDF.");
  if (/blocked/i.test(message)) return t("Captions are temporarily unavailable. Try a PDF or try again later.");
  if (/25 MB/i.test(message)) return t("Choose a PDF smaller than 25 MB.");
  if (/network|fetch|timed out|aborted/i.test(message)) return t("The connection failed. Your source is still here — you can try again.");
  return (message && message !== 'unknown' ? message : '') || t("Something went wrong. Please try again.");
}
...
export default function App() {
  ...
  const suggestions = (["Summarize the essentials", "Explain simply", "Key takeaways"] as const).map(t);
  const statuses = { ended: t("At your own pace"), connecting: t("Connecting…"), connected: t("Session active"), reconnecting: t("Reconnecting…"), error: t("Connection interrupted") };
  ...
  const notice = error ? <Reveal motion={motion} style={s.error}><Text accessibilityRole="alert" style={s.errorText}>{messageOf(error, t)}</Text><Touch label={t("Dismiss message")} motion={motion} onPress={() => setError('')}><Text style={s.errorText}>×</Text></Touch></Reveal> : null;
  ...
  return <SafeAreaProvider><StatusBar barStyle="dark-content" /><SafeAreaView style={s.screen} edges={['top', 'left', 'right', 'bottom']}>
    <KeyboardAvoidingView style={s.screen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      {screen === 'home' ? <ScrollView key="home" contentContainerStyle={s.homeContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={s.header}><Brand /><Touch label={t("About Ursly and language")} onPress={() => setSheet('about')} motion={motion} style={s.infoButton}><Text style={s.tabLabel}>{language.toUpperCase()}</Text></Touch></View>
        <Reveal motion={motion} style={s.hero}>
          <View style={s.rowBetween}><Text style={s.heroEyebrow}>{t("A LITTLE MORE CLARITY")}</Text><View style={s.smallDot} /></View>
          <View style={s.heroMain}><Text style={s.heroTitle}>{t("Your ideas.")}{ '\n' }<Text style={{ color: c.lime }}>{t("Made clear.")}</Text></Text><View style={s.heroArt}><Orbit motion={motion} /></View></View>
          <Text style={s.heroDescription}>{t("A document. A video.")}{ '\n' }{t("And the conversation begins.")}</Text>
          <View style={s.heroFooter}><View style={s.heroLine} /><Text style={s.heroFooterText}>{t("Less scrolling. More understanding.")}</Text></View>
        </Reveal>
        <Reveal motion={motion} delay={70}>
          <View style={s.sectionHeading}><Text style={s.heading}>{t("Let’s explore")}</Text><Text style={s.sectionNote}>{t("Your choice")}</Text></View>
          <View style={s.importRow}>
            <Touch label={t("Import a PDF")} onPress={() => void ingest('pdf')} motion={motion} disabled={!!busy} style={[s.importCard, { backgroundColor: c.peach }]}>
              <View style={s.importInner}><View style={s.rowBetween}><View style={s.iconTile}><SourceIcon kind="pdf" /></View><Text style={s.diagonalArrow}>↗</Text></View><View><Text style={s.importTitle}>{t("A document")}</Text><Text style={s.importCaption}>{t("PDF · up to 25 MB")}</Text></View></View>
            </Touch>
            <Touch label={t("Add a YouTube video")} onPress={() => { setError(''); setSheet('youtube'); }} motion={motion} disabled={!!busy} style={[s.importCard, { backgroundColor: c.lavender }]}>
              <View style={s.importInner}><View style={s.rowBetween}><View style={s.iconTile}><SourceIcon kind="youtube" /></View><Text style={s.diagonalArrow}>↗</Text></View><View><Text style={s.importTitle}>{t("A video")}</Text><Text style={s.importCaption}>{t("Paste a YouTube link")}</Text></View></View>
            </Touch>
          </View>
        </Reveal>
        ...
        {source ? <Reveal motion={motion} delay={100}><Touch label={t("Resume conversation")} onPress={() => setScreen('conversation')} motion={motion} disabled={!!busy} style={s.resume}><View style={s.resumeInner}><View style={s.resumeIcon}><SourceIcon kind={source.kind} /></View><View style={s.flex}><Text style={s.eyebrow}>{t("PICK UP WHERE YOU LEFT OFF")}</Text><Text numberOfLines={2} style={s.resumeTitle}>{source.sourceName}</Text></View><Text style={s.diagonalArrow}>→</Text></View></Touch></Reveal> :
          <Reveal motion={motion} delay={140}><Touch label={t("Try a sample text")} onPress={trySample} motion={motion} disabled={!!busy} style={s.sample}><View style={s.resumeInner}><View style={s.sampleStar}><Text style={s.star}>✦</Text></View><View style={s.flex}><Text style={s.sampleTitle}>{t("A little taste of Ursly")}</Text><Text style={s.caption}>{t("Explore Ursly with a short read.")}</Text></View><Text style={s.diagonalArrow}>→</Text></View></Touch></Reveal>}
        <Reveal motion={motion} delay={180} style={s.how}><Text style={s.eyebrow}>{t("A NEW WAY TO LEARN")}</Text><View style={s.steps}>{[['01', t("Add")], ['02', t("Ask")], ['03', t("Understand")]].map(([number, label]) => <View key={number} style={s.step}><Text style={s.stepNumber}>{number}</Text><Text style={s.stepText}>{label}</Text></View>)}</View></Reveal>
        <Text style={s.signature}>{t("Make room for your next aha.")}</Text>
        {mode === 'mock' && <Text style={s.demoFootnote}>{t("Demo space · simulated answers and audio")}</Text>}
      </ScrollView> : <>
        <View style={s.workspaceHeader}><Touch label={t("Back to sources")} onPress={() => { stop(); setScreen('home'); Keyboard.dismiss(); }} motion={motion} disabled={!!busy}><Text style={s.back}>‹ <Text style={s.backLabel}>{t("Sources")}</Text></Text></Touch><Brand small /><Touch label={t("About Ursly and language")} onPress={() => setSheet('about')} motion={motion} style={{ width: 84 }}><Text style={s.tabLabel}>{language.toUpperCase()}</Text></Touch></View>
        <View style={s.tabs}>{(['chat', 'source'] as const).map(value => <Touch key={value} label={value === 'chat' ? t("Conversation") : t("The source")} onPress={() => { setTab(value); Keyboard.dismiss(); }} selected={tab === value} motion={motion} style={[s.tab, tab === value && s.tabSelected]}><Text style={[s.tabLabel, tab === value && { color: c.ink }]}>{value === 'chat' ? t("Conversation") : t("The source")}</Text></Touch>)}</View>
        {active && <View style={s.pinnedVoice}><Text accessibilityLiveRegion="polite" style={s.statusText}>{muted ? t("Microphone muted") : statuses[status]}</Text><View style={s.voiceButtons}><Touch label={muted ? t("Unmute microphone") : t("Mute microphone")} motion={motion} onPress={() => { voice.current?.setMuted(!muted); setMuted(!muted); }} disabled={status !== 'connected'} style={s.secondaryDark}><Text style={s.buttonLight}>{muted ? t("Unmute") : t("Mute")}</Text></Touch><Touch label={t("Stop session")} motion={motion} onPress={stop} style={s.primary}><Text style={s.buttonInk}>{t("■  End session")}</Text></Touch></View><Touch label={t("Continue by typing")} motion={motion} onPress={() => { stop(); composer.current?.focus(); }}><Text style={s.textLinkLight}>{t("Continue by typing")}</Text></Touch></View>}
        <ScrollView key={tab} ref={scroll} contentContainerStyle={s.chatContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}
          ...
          onScroll={({ nativeEvent: { contentOffset, contentSize, layoutMeasurement } }) => { followTranscript.current = contentOffset.y + layoutMeasurement.height >= contentSize.height - 80; }}
          onContentSizeChange={() => { if (tab === 'chat' && turns.length && followTranscript.current) scroll.current?.scrollToEnd({ animated: motion }); }}>
          {source && <Reveal motion={motion} style={s.sourceSummary}><View style={s.sourceBadge}><SourceIcon kind={source.kind} /></View><View style={s.flex}><Text style={s.eyebrow}>{source.kind === 'youtube' ? t("YOUTUBE VIDEO") : t("YOUR DOCUMENT")}</Text><Text style={s.sourceName} numberOfLines={tab === 'chat' ? 2 : undefined}>{source.sourceName}</Text><Text style={s.caption}>{source.characters.toLocaleString(language === 'en' ? 'en-CA' : 'fr-CA')}{language === 'en' ? ' characters · ready to explore' : ' caractères · prêt à explorer'}</Text></View></Reveal>}
          {tab === 'source' ? <Reveal motion={motion} delay={60} style={s.readingCard}><Text style={s.readingTitle}>{t("It all starts here.")}</Text><Text style={s.readingHint}>{t("Your source text, always close at hand.")}</Text><Text selectable style={s.sourceText}>{expanded ? source?.text : (source?.text.slice(0, 600) ?? '') + ((source?.text.length ?? 0) > 600 ? '…' : '')}</Text>{(source?.text.length ?? 0) > 600 && <Touch label={expanded ? t("Show less") : t("Read the full text")} motion={motion} onPress={() => setExpanded(!expanded)} style={s.outline}><Text style={s.buttonInk}>{expanded ? t("Show less ↑") : t("Read the full text ↓")}</Text></Touch>}<Touch label={t("Ask about this source")} motion={motion} onPress={() => setTab('chat')} style={s.primary}><Text style={s.buttonInk}>{t("Let’s talk →")}</Text></Touch></Reveal> : <>
            <Reveal motion={motion} delay={65} style={s.voiceCard}>
              <View style={s.rowBetween}><View style={s.flex}><Text style={s.voiceEyebrow}>{t("THE JOY OF UNDERSTANDING")}</Text><Text style={s.voiceTitle}>{active ? t("Let’s talk it through.") : t("Think out loud.")}</Text></View><Wave motion={motion && active && !muted} color={c.coral} large /></View>
              <View style={s.statusRow}><View style={[s.statusDot, { backgroundColor: status === 'error' ? c.coral : active ? c.lime : c.lilac }]} /><Text accessibilityLiveRegion="polite" style={s.statusText}>{muted ? t("Microphone muted") : statuses[status]}</Text>{(status === 'connecting' || status === 'reconnecting') && <ActivityIndicator size="small" color={c.lime} />}</View>
              {!active &&
                <Touch label={t("Start voice conversation")} motion={motion} onPress={startVoice} disabled={!!busy} style={s.primary}><View style={s.buttonRow}><Wave motion={false} /><Text style={s.buttonInk}>{t("Let’s talk")}</Text><Text style={s.arrow}>↗</Text></View></Touch>}
              {mode === 'mock' && <Text style={s.demoNotice}>{t("Demo mode · voice and answers are simulated.")}</Text>}
            </Reveal>
            {notice}
            {!turns.length && <Reveal motion={motion} delay={100} style={s.conversationEmpty}><Text style={s.emptyTitle}>{t("The best question")}{ '\n' }{t("is yours.")}</Text><Text style={s.emptyCaption}>{t("A detail to clarify, an idea to explore…")}</Text><View style={s.suggestions}>{suggestions.map((prompt, index) => <Touch key={prompt} label={prompt} motion={motion} disabled={!!busy} onPress={() => { setQuestion(prompt); composer.current?.focus(); }} style={s.suggestion}><View style={s.suggestionInner}><Text style={[s.suggestionSymbol, { color: ['#A9513A', '#71608D', '#536C39'][index] }]}>{['✦', '≈', '↗'][index]}</Text><Text style={s.suggestionText}>{prompt}</Text><Text style={s.suggestionArrow}>+</Text></View></Touch>)}</View></Reveal>}
            {turns.map(turn => <Reveal key={turn.id} motion={motion} style={[s.message, turn.role === 'user' ? s.userMessage : s.assistantMessage]}><View style={s.messageHeader}>{turn.role === 'assistant' && <View style={s.miniBrand}><Wave motion={false} color={c.coral} /></View>}<Text style={s.messageRole}>{turn.role === 'user' ? t("YOU") : 'URSLY'}</Text></View><Text selectable style={s.messageText}>{turn.text}</Text></Reveal>)}
            {busy === 'chat' && <View style={s.loading} accessibilityLiveRegion="polite"><ActivityIndicator color={c.coral} /><Text style={s.caption}>{t("Ursly is preparing an answer…")}</Text></View>}
          </>}
        </ScrollView>
        {tab === 'chat' && <View style={s.composerWrap}><View style={s.composer}><TextInput ref={composer} accessibilityLabel={t("Your question")} placeholder={t("What’s on your mind?")} placeholderTextColor={c.muted} value={question} onChangeText={setQuestion} multiline style={s.questionInput} editable={!busy} /><Touch label={t("Send question")} motion={motion} onPress={() => void ask()} disabled={!!busy || !question.trim()} style={s.send}>{busy === 'chat' ? <ActivityIndicator color={c.ink} /> : <Text style={s.sendArrow}>↑</Text>}</Touch></View><Text style={s.composerHint}>{t("Grounded in your source. Explored through your eyes.")}</Text></View>}
      </>}
    ...
    <Modal visible={sheet !== null} transparent animationType={motion ? 'slide' : 'none'} onRequestClose={() => { if (!busy) setSheet(null); }}>
      <KeyboardAvoidingView style={s.modalRoot} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <Pressable accessibilityRole="button" accessibilityLabel={t("Close window")} style={s.scrim} disabled={!!busy} onPress={() => setSheet(null)} />
        <SafeAreaView style={s.sheet} edges={['bottom']}><ScrollView style={{ flexShrink: 1 }} contentContainerStyle={{ gap: 18, paddingBottom: 8 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><View style={s.sheetHandle} /><View style={s.rowBetween}><Text style={s.sheetTitle}>{sheet === 'youtube' ? t("One video. New ideas.") : t("Hi there, we’re Ursly.")}</Text><Touch label={t("Close")} onPress={() => setSheet(null)} motion={motion} disabled={!!busy}><Text style={s.close}>×</Text></Touch></View>
          {sheet === 'about' && <View style={{ gap: 10 }}><Text style={s.aboutTitle}>{t("App language")}</Text><View style={{ flexDirection: 'row', gap: 8 }}>{(['en', 'fr'] as const).map(value => <Touch key={value} label={value === 'en' ? 'English' : 'Français'} selected={language === value} motion={motion} onPress={() => setLanguage(value)} style={[s.tab, { backgroundColor: language === value ? c.lavender : c.white }]}><Text style={s.tabLabel}>{value === 'en' ? 'English' : 'Français'}{language === value ? ' ✓' : ''}</Text></Touch>)}</View></View>}
          {sheet === 'youtube' ? <><Text style={s.sheetCaption}>{t("Paste a YouTube link with captions.")}{ '\n' }{t("We’ll get the text. You bring the curiosity.")}</Text><TextInput accessibilityLabel={t("YouTube link")} placeholder="https://youtube.com/watch?v=…" placeholderTextColor={c.muted} value={url} onChangeText={setUrl} autoCapitalize="none" autoCorrect={false} keyboardType="url" returnKeyType="go" onSubmitEditing={() => { if (url.trim()) void ingest('youtube'); }} style={s.urlInput} editable={!busy} />{notice}<Touch label={t("Load video")} motion={motion} onPress={() => void ingest('youtube')} disabled={!!busy || !url.trim()} style={s.primary}><View style={s.buttonRow}>{busy === 'youtube' && <ActivityIndicator color={c.ink} />}<Text style={s.buttonInk}>{busy === 'youtube' ? t("Getting things ready…") : t("Explore this video →")}</Text></View></Touch></> : <><Text style={s.sheetCaption}>{t("Your documents and videos have something to say. Ursly helps you explore them, one question at a time.")}</Text><View style={s.aboutNote}><Text style={s.aboutTitle}>{t("Your curiosity takes it from here.")}</Text><Text style={s.body}>{t("Add a source, read its text and chat by typing or speaking. Conversation history stays in the current session.")}</Text></View><Text style={s.caption}>{mode === 'mock' ? t("This space uses demo answers, captions and audio.") : t("Voice requires microphone access and a connected service.")}</Text><Text style={s.caption}>{t("Animations follow your device’s accessibility preferences.")}</Text></>}
...
const s = StyleSheet.create({
  ...
  pinnedVoice: { backgroundColor: c.ink, marginHorizontal: 22, padding: 12, gap: 6, borderRadius: 20 },
