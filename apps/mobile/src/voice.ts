...
export class NativeVoice {
  ...
  async start() {
    ...
    this.timer = setTimeout(
      () =>
        this.fail("Voice connection timed out. Try again or use text chat."),
      30000,
    );
    try {
      ...
      if (this.stopped) return;
      const stream = await native.mediaDevices.getUserMedia({
        audio: true,
        video: false,
      });
      if (this.stopped) {
      ...
      channel.onmessage = (message: { data: unknown }) => {
        ...
        try {
          ...
          if (event.type === "error")
            this.error(event.error?.message || "Voice service error.");
          this.event(event);
        } catch {
          this.error("An unreadable voice event was received.");
        }
      };
      peer.onconnectionstatechange = () => {
        ...
        if (peer.connectionState === "disconnected") {
          ...
          this.timer = setTimeout(
            () =>
              this.fail("Connection lost. Retry voice or continue in text."),
            15000,
          );
        }
        ...
        if (peer.connectionState === "failed")
          this.fail("Voice connection failed. Retry or use text chat.");
      };
      ...
      await peer.setRemoteDescription(
        new native.RTCSessionDescription({ type: "answer", sdp: answer }),
      );
    } catch (error) {
      if (!this.stopped)
        this.fail(
          error instanceof Error
            ? error.message
            : "Microphone unavailable. Use text chat.",
        );
    }
  }
