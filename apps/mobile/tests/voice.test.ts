...
test("mock voice starts and stops without requesting the native microphone", async () => {
  ...
  const api = new ApiClient(
    "http://localhost",
    async () => new Response(JSON.stringify({ mode: "mock" })),
  );
...
test("duplicate starts and stops are idempotent", async () => {
  let sessions = 0;
  const statuses: string[] = [];
  const api = new ApiClient("http://localhost", async () => {
    sessions += 1;
    return new Response(JSON.stringify({ mode: "mock" }));
  });
  const voice = new NativeVoice(
    api,
    source,
