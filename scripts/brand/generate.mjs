...
const wave = (color) =>
  bars
    ...
    .join("");
const svg = (body) =>
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="1024" height="1024">${body}</svg>`;
const mark = svg(
  `<rect width="100" height="100" rx="30" fill="${coral}"/>${wave(ink)}`,
);
...
for (const directory of ["apps/mobile/assets", "apps/web/public/brand"])
  await mkdir(directory, { recursive: true });
async function png(source, size, path) {
  const canvas = createCanvas(size, size);
  canvas
    .getContext("2d")
    .drawImage(await loadImage(Buffer.from(source)), 0, 0, size, size);
...
for (const size of [192, 512])
  await png(app, size, `apps/web/public/brand/icon-${size}.png`);
// PNG-backed ICO directory with crisp small-size variants.
const sizes = [16, 32, 48];
const entries = await Promise.all(
  sizes.map((size) =>
    png(mark, size, `apps/web/public/brand/favicon-${size}.png`),
  ),
);
...
ctx.fillStyle = "#E8E1F5";
ctx.beginPath();
ctx.arc(1110, 70, 285, 0, Math.PI * 2);
ctx.fill();
ctx.drawImage(await loadImage(Buffer.from(mark)), 76, 64, 88, 88);
ctx.fillStyle = ink;
ctx.font = "bold 76px Arial";
ctx.fillText("ursly", 184, 133);
ctx.fillStyle = coral;
ctx.fillText(".", 367, 133);
ctx.fillStyle = ink;
ctx.font = "bold 78px Georgia";
ctx.fillText("The joy of", 76, 294);
ctx.fillText("understanding.", 76, 385);
ctx.font = "28px Arial";
ctx.fillText("Your sources. Your questions. A real conversation.", 80, 464);
ctx.font = "bold 23px Arial";
ctx.fillText("ursly.io", 80, 557);
ctx.drawImage(await loadImage(Buffer.from(mark)), 906, 345, 180, 180);
await writeFile(
  "apps/web/public/brand/social-card.png",
  card.toBuffer("image/png"),
);
