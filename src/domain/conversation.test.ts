...
describe("conversation reducer", () => {
  it("models connection lifecycle", () => {
    let state = conversationReducer(initialConversationState, { type: "PREPARING" });
  ...
  it("accumulates partial assistant transcript events", () => {
    ...
    state = conversationReducer(state, { type: "MESSAGE_DELTA", id: "m2", text: "Hi" });
    state = conversationReducer(state, { type: "MESSAGE_DELTA", id: "m2", text: " there" });
    expect(state.messages[0].text).toBe("Hi there");
  });

  it("toggles microphone mute state", () => {
    const state = conversationReducer(initialConversationState, { type: "MUTE_CHANGED", muted: true });
