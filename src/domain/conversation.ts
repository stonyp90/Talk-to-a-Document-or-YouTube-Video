export type ConnectionStatus = "idle" | "preparing" | "connecting" | "connected" | "reconnecting" | "ended" | "error";
...
export const initialConversationState: ConversationState = { status: "idle", muted: false, messages: [] };
  ...
  switch (event.type) {
    ...
    case "MESSAGE_DELTA":
      return {
        ...
        messages: state.messages.map((message) =>
          message.id === event.id ? { ...message, text: `${message.text}${event.text}`, status: "partial" } : message,
        ),
      };
    case "MESSAGE_COMPLETED":
      return {
        ...state,
        messages: state.messages.map((message) =>
          message.id === event.id ? { ...message, text: event.text ?? message.text, status: "complete" } : message,
        ),
      };
  }
}
