...
it("extracts real PDF contents",async()=>{
 ...
 const result=await extractPdfText(new File([bytes],"source.pdf",{type:"application/pdf"}));
 expect(result.text).toContain("The source says forty two.");
});
it("rejects renamed non-PDF bytes",async()=>{
 await expect(extractPdfText(new File(["hello"],"source.pdf",{type:"application/pdf"}))).rejects.toThrow("not a valid PDF");
});
