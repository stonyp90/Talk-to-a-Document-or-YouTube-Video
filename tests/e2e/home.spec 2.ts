...
test.describe("source conversation journey", () => {
  test("ingests a YouTube transcript and uses text fallback chat", async ({ page }) => {
    await page.goto("/");
    ...
    await expect(page.getByText(/deterministic local transcript/)).toBeVisible();

    await page.getByLabel("Ask a question").fill("What is this about?");
    await page.getByRole("button", { name: "Send" }).click();
    await expect(page.getByText(/Local demo response/)).toBeVisible();
  });

  test("starts a local mock voice session and supports mute/stop", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("tab", { name: "YouTube video" }).click();
